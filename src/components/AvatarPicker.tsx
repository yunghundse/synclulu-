...
// Delulu-style avatar options with actual images
const AVATARS = [
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50 p-4">
      <div className="bg-white rounded-3xl w-full max-w-md p-6 animate-slide-up">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          {AVATARS.map((avatar) => (
            <button
              ...
              className={`relative aspect-square rounded-2xl bg-gradient-to-br from-delulu-soft to-white p-2 transition-all ${
                selected === avatar.id
                  ? 'scale-105 ring-4 ring-delulu-violet ring-offset-2 shadow-lg'
            ...
            >
              ...
              />
              {selected === avatar.id && (
                <div className="absolute -top-2 -right-2 w-7 h-7 bg-delulu-violet rounded-full flex items-center justify-center shadow-lg">
        ...
        <div className="text-center mb-6">
          <p className="text-sm text-delulu-muted">
            {t('avatar.selected')}: <span className="font-semibold text-delulu-text">
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-3 rounded-xl border-2 border-delulu-soft font-semibold text-delulu-muted"
          >
            {t('common.cancel')}
          </button>
          <button
...
export const Avatar = ({
  ...
  return (
    <div
      className={`rounded-2xl bg-gradient-to-br from-delulu-soft to-white flex items-center justify-center p-1 ${sizeClasses[size]} ${className}`}
    >
      <img
        src={avatar.image}
        alt={avatar.name}
