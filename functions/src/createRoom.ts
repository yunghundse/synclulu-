...
function isFounder(uid: string): boolean {
...
async function findNearbyRoom(
  ...
  for (const doc of roomsSnapshot.docs) {
    ...
    // Skip full rooms
    if ((data.participants?.length || 0) >= (data.maxParticipants || 8)) {
      continue;
    }
...
export const createRoom = functions
  ...
  .https.onCall(async (data: CreateRoomRequest, context) => {
    // Auth check
    if (!context.auth) {
    ...
    try {
      ...
      if (data.location && !data.isGhostMode) {
        const nearbyRoom = await findNearbyRoom(
          data.location,
          data.type,
          data.isAnonymous
        );
        ...
        if (nearbyRoom) {
          ...
          await db.runTransaction(async (transaction) => {
            ...
            const alreadyIn = participants.some((p: any) => p.oderId === uid);
            if (alreadyIn) {
      ...
      if (data.location) {
        roomData.location = new admin.firestore.GeoPoint(
          data.location.lat,
          data.location.lng
        );
      }
...
export const cleanupStaleRooms = functions
  .region('europe-west1')
  ...
  .onRun(async () => {
    ...
    for (const roomDoc of roomsSnapshot.docs) {
      ...
      const activeParticipants = participants.filter((p: any) => {
        const lastActive = p.lastActiveAt?.toMillis?.() || 0;
      ...
      if (activeParticipants.length === 0) {
        ...
        functions.logger.info(
          `[Cleanup] Cleaned ${participants.length - activeParticipants.length} stale participants from: ${roomDoc.id}`
        );
      }
    }

    functions.logger.info(
      `[Cleanup] Complete: ${deletedCount} rooms deleted, ${cleanedCount} rooms cleaned`
    );
...
export const createGhostRoom = functions
  .region('europe-west1')
  .https.onCall(async (data: Omit<CreateRoomRequest, 'location' | 'isGhostMode'>, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError('unauthenticated', 'Nicht eingeloggt');
    }
    ...
    const roomData = {
      name: data.name?.trim() || 'Ghost Wölkchen',
